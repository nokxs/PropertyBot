...
namespace PropertyBot.Provider.OhneMakler.WebClient
{
    internal class ImmosuchmaschineWebClient : IImmosuchmaschineWebClient
    {
        ...
        {
            return await _paginationHelper.GetPaginatedObjects(GetRawPage, ParseHtml, GetPageCount, options);
        }
        ...
        private IEnumerable<ImmosuchmaschineProperty> ParseHtml(string htmlString)
        {
            ...
            return propertyNodes?.Where(node => node.ChildNodes.Count == 7).Select(ConvertToProperty).ToList() ?? Enumerable.Empty<ImmosuchmaschineProperty>();
        }
