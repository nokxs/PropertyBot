...
{
    internal class ImmoscoutListWebClient : IImmoscoutListWebClient
    {
        private readonly HttpClient _client;

        public ImmoscoutListWebClient()
        {
            _client = new HttpClient();
        }

        public async Task<IEnumerable<ImmoscoutListProperty>> GetObjects(ImmoscoutListWebClientOptions options)
        {
        ...
        private IEnumerable<ImmoscoutListProperty> ParseHtml(string htmlString)
        {
            ...
            return propertyNodes?.Select(ConvertToProperty).ToList() ?? Enumerable.Empty<ImmoscoutListProperty>();
        }

        private ImmoscoutListProperty ConvertToProperty(HtmlNode node)
        {
            var htmlDoc = new HtmlDocument();
            htmlDoc.LoadHtml(node.OuterHtml);

            return new ImmoscoutListProperty(
